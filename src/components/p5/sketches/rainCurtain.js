// AUTO-GENERATED from rainCurtain.raw.js by scripts/p5-transform.mjs — do not edit by hand.
// Source template: p5-animation skill (CC BY-NC 4.0, xxoogreymon).
/* eslint-disable */
export function makeRainCurtain(container) {
  return function ($p) {
...
// —— 帘线 ——
const CURTAIN_LINE_COUNT = 32;
const CURTAIN_STROKE_WEIGHT = 1;
const CURTAIN_COLOR = "#A39B91";
const CURTAIN_STROKE_ALPHA = 128;
const CURTAIN_LENGTH = 750;

// —— 雨滴 ——
const DROPS_PER_LINE_MIN = 5;
const DROPS_PER_LINE_MAX = 12;
const DROP_START_MIN = 30;
const DROP_START_MAX = 80;
const DROP_SIZE_MIN = 0.25;
const DROP_SIZE_MAX = 0.55;
const DROP_POS_JITTER = 6;

// —— Verlet 物理 ——
const VERLET_TIMESTEPS = 40;
const VERLET_GRAVITY = 0.18;
const VERLET_AIR_DRAG = 0.995;
const VERLET_MASS = 1.0;
const VERLET_REPULSION_SIZE = 100;
...
// —— 撩动脱落 ——
const DETACH_MOUSE_SPEED_MIN = 1.2;
const DETACH_BEAD_SPEED_MIN = 1.4;
const DETACH_PROBABILITY = 0.18;
const DETACH_REPULSE_MULT = 1.45;
const MAX_DETACH_PER_FRAME = 3;
const FALLING_DROP_GRAVITY = 0.55;
...
VerletPoint.prototype.sim = function () {
  ...
  var d = dist($p.mouseX, $p.mouseY, this.pos.x, this.pos.y);
  if (d < VERLET_REPULSION_SIZE && d > 1e-6) {
    var repulse = this.pos.copy().sub(createVector($p.mouseX, $p.mouseY));
    repulse.normalize();
...
function maybeDetachDropsFromAgitation() {
  ...
  var mspd = dist($p.mouseX, $p.mouseY, $p.pmouseX, $p.pmouseY);
  var mouseFast = mspd >= DETACH_MOUSE_SPEED_MIN;
  var mouseDrag = $p.mouseIsPressed && $p.mouseButton === LEFT && mspd > 0.35;
  ...
  for (var bi = 0; bi < curtainBodies.length; bi++) {
    ...
    for (var k = 0; k < rb.drops.length; k++) {
      ...
      if (dist($p.mouseX, $p.mouseY, p.pos.x, p.pos.y) > r0) continue;
...
function stepFallingDrops() {
  for (var i = fallingDrops.length - 1; i >= 0; i--) {
    ...
    f.x += f.vx;
    f.y += f.vy;
    if (f.y > $p.height + 100 || f.x < -120 || f.x > $p.width + 120) {
...
function draw() {
  ...
  if (curtainBodies.length === 0) {
    ...
    textSize(16);
    text("loading...", $p.width / 2, $p.height / 2);
    return;
  }
